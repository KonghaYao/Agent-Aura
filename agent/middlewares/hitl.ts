...
import type { JumpToTarget } from "langchain";
...
const DescriptionFunctionSchema = z.function({
    input: z.tuple([
        // 修复：添加 validator（检查是否为非空对象），并自定义错误
        z.custom<ToolCall>(),
        z.custom<AgentBuiltInState>(),
        z.custom<Runtime<unknown>>(),
    ]),
    output: z.union([z.string(), z.promise(z.string())]), // 输出：字符串或 Promise<string>
});
            ...
            hook: async (state, runtime) => {
                ...
                for (const [toolName, toolConfig] of Object.entries(
                    ...
                    if (typeof toolConfig === "boolean") {
                        ...
                        resolvedConfigs[toolName] =
                            toolConfig as InterruptOnConfig;
                    }
                }
                ...
                for (const toolCall of interruptToolCalls) {
                     ...
                     */
                    const { actionRequest, reviewConfig } =
                        await createActionAndConfig(
                            toolCall,
                            interruptConfig,
                            state,
                            runtime,
                ...
                for (let i = 0; i < decisions.length; i++) {
                    ...
                    if (
                        revisedToolCall &&
                        /**
                         * If any decision is a rejected or replied, we are going back to the model
                         * with only the tool calls that were handled as we don't know
                ...
                const jumpTo: JumpToTarget | undefined = hasHandledToolCalls
                    ? "model"
                    : undefined;
                return {
